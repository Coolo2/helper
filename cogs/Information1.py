...
class Information1(commands.Cog):
    ...
    @bot.command(name="help", description="Get help on the bot and it's commands")
    ...
    async def help(self, ctx : discord.Interaction, command : str = None):
        if command == None:
            embed = discord.Embed(
                ...
                description=f"Use **/help [command]** to get help on a specific command.",
                colour=var.embed
            )
            ...
            for cmd in commands:
                if cmd.name.lower() == command.lower().replace("/", ""):
                    if cmd.name.lower() == command.lower().replace("/", ""):
                        found = True
                        
                    for category in data:
                        if cmd.name == command.replace("/", ""):
                    ...
                    if found == True:
                        ...
                        embed.add_field(name="Usage", value="/" + cmd.name + optionString, inline=False)
                        try:
...
[Dashboard]({var.address}/dashboard/{(str(ctx.guild.id)) if ctx.guild else ''})
    ...
    async def _changelogs(self, interaction : discord.Interaction):
        ...
        embed = discord.Embed(title=f"Changelogs up to {var.version}")
